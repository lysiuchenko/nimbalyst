...
describe('buildClaudeCliSpawnConfig', () => {
  ...
  it('defaults ENABLE_TOOL_SEARCH to true so MCP tool schemas always defer (parity with the Agent SDK path)', () => {
  ...
  it('lets the user override ENABLE_TOOL_SEARCH from their own env (default does not clobber it)', () => {
    const cfg = buildClaudeCliSpawnConfig({
      ...base,
      baseEnv: { ENABLE_TOOL_SEARCH: 'false' },
    });
    expect(cfg.env.ENABLE_TOOL_SEARCH).toBe('false');
  });
