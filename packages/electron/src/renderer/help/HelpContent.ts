...
export const HelpContent: Record<string, HelpEntry> = {
  ...
  'org-window-status-bar': {
    title: 'Glue Teams is in beta',
    body: 'Expect bugs.\n\nOrganizations are free during beta and will require a paid Glue Teams subscription after launch; existing organizations get advance notice before any pricing change.',
  },
  ...
  'file-tree-refresh-button': {
    ...
    body: 'Reload the file list from disk to pick up files added or removed outside Glue.',
  },
...
**Agent**: Show only the conversation transcript.`,
  ...
  'tracker-automation-section': {
    ...
    body: `Automatically connect git commits to your tracker items. When enabled, Glue links commits via the session's tracker items and by parsing issue keys (e.g. **NIM-123**) from commit messages — including commits made in your terminal.\n\nFor project-specific behavior, add instructions to your project's **CLAUDE.md** (e.g. "always reference tracker issue keys in commits" or "don't auto-close critical bugs without review").`,
  },
  ...
  'model-picker-provider-claude-code': {
    ...
    body: 'The in-app agent built on Claude Code with full Glue integration: it sees your active document and selection, renders the rich inline transcript, and tracks every file it edits. Uses your configured Anthropic API key.',
  },
  'model-picker-provider-claude-code-cli': {
    title: 'Claude Code CLI (Terminal Mode)',
    body: 'Runs the genuine claude terminal binary in an embedded terminal, billed to your Claude subscription. You get native CLI behavior — its slash commands and TUI — in the Raw terminal drawer, while Glue mirrors the conversation into the rich transcript.',
  },
