...
function file(path: string, extra: Partial<FlowFile> = {}): FlowFile {
  return { flowPath: path, flowName: path.replace('.flow.json', ''), schedule: null, ...extra };
}
...
describe('buildFlowRows', () => {
  ...
  test('carries the schedule from the flow file', () => {
    const rows = buildFlowRows(
      [file('nightly.flow.json', { schedule: { type: 'daily', time: '02:00', enabled: true } })],
      []
    );

    expect(rows[0].schedule).toEqual({ type: 'daily', time: '02:00', enabled: true });
  });
